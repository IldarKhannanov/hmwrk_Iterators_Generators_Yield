...
class FlatIterator:
    ...
    def __next__(self):
        self.item_index += 1
        if self.item_index > self.res.index(self.res[-1]):
            raise StopIteration
        return self.res[self.item_index]
...
for i in FlatIterator(check_list):
    print(i)
...
# проверяем работу функции
chek_list = [['a', 'b', 'c'], ['d', 'e', 'f', 'h', False], [1, 2, None]]

for i in flat_generator(chek_list):
    print(i)